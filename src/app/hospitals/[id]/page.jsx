...
export default function HospitalProfilePage({ params }) {
  // Mock Data for CityCare Hospital
  const hospital = {
    id: params?.id || "citycare",
    name: "CityCare Hospital",
    isVerified: true,
    rating: 4.7,
    reviewsCount: 88,
    location: "Plot 102, Ahmadu Bello Way, Central Business District, Abuja",
    ...
    facilities: ["Parking", "Pharmacy", "Laboratory", "Emergency unit"],
    reviews: [
      {
        id: "rev-1",
        patientName: "Bisi A.",
      ...
      <div className="max-w-4xl mx-auto space-y-6">
              ...
              <div className="flex items-center gap-2">
                ...
                {hospital.isVerified && (
                  <span className="inline-flex items-center gap-1 text-xs font-bold text-teal-700 bg-teal-50 border border-teal-200 px-2 py-0.5 rounded-full">
                    <BadgeCheck className="w-3.5 h-3.5 text-teal-600" /> Verified Healthcare Facility
                  </span>
                )}
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-3">
            <h2 className="text-xs font-extrabold uppercase tracking-wider text-slate-900 flex items-center gap-2 border-b border-slate-100 pb-3">
              <Stethoscope className="w-4 h-4 text-teal-600" /> Medical Services
            </h2>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-3">
            <h2 className="text-xs font-extrabold uppercase tracking-wider text-slate-900 flex items-center gap-2 border-b border-slate-100 pb-3">
              <Building className="w-4 h-4 text-teal-600" /> On-site Facilities
            </h2>
            <div className="flex flex-wrap gap-2 pt-1">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-4">
          <h2 className="text-sm font-extrabold text-slate-900 border-b border-slate-100 pb-3 flex items-center gap-2">
            <UserCheck className="w-4 h-4 text-teal-600" /> Attending Specialists & Doctors
          </h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {hospital.doctors.map((doc) => (
              ...
              >
                <h3 className="font-extrabold text-xs text-slate-900">{doc.name}</h3>
                <p className="text-[11px] font-medium text-teal-700">{doc.specialty}</p>
                <p className="text-[10px] text-slate-400 pt-1">{doc.experience}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-4">
          <h2 className="text-sm font-extrabold text-slate-900 border-b border-slate-100 pb-3 flex items-center gap-2">
            <MessageSquare className="w-4 h-4 text-teal-600" /> Patient Reviews
          </h2>
